...
lint:  ## run static analysis with flake8
	python -m black --check cryptorl setup.py
	python -m flake8 cryptorl setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black cryptorl/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./cryptorl

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v cryptorl/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v cryptorl/tests --cov=cryptorl --cov-branch --cov-fail-under=75 --cov-report term-missing
